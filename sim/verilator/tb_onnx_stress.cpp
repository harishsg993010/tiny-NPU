// =============================================================================
// tb_onnx_stress.cpp - Stress test for Graph Mode (large graph, 5M cycle timeout)
// Loads program/tdesc/ddr_image from files, runs graph pipeline,
// handles DMA transfers in C++, compares output against golden.
// Prints ALL performance counters in a formatted table at completion.
// =============================================================================
#include <cstdio>
#include <cstdlib>
#include <cstdint>
...
// Helpers
// =============================================================================
static std::string g_datadir = "build/graph_stress";
    ...
    // ---- Run until done or timeout ----
    {
        ...
        while (timeout-- > 0 && !done) {
            ...
            if (dut->graph_done) {
                ...
                // Print ALL performance counters in a formatted table
                printf("\n");
                printf("  +---------------------------+------------+\n");
                printf("  | Performance Counter       |     Cycles |\n");
                printf("  +---------------------------+------------+\n");
                printf("  | perf_total_cycles         | %10u |\n", dut->perf_total_cycles);
                printf("  | perf_gemm_cycles          | %10u |\n", dut->perf_gemm_cycles);
                printf("  | perf_softmax_cycles       | %10u |\n", dut->perf_softmax_cycles);
                printf("  | perf_dma_cycles           | %10u |\n", dut->perf_dma_cycles);
                printf("  | perf_reduce_cycles        | %10u |\n", dut->perf_reduce_cycles);
                printf("  | perf_math_cycles          | %10u |\n", dut->perf_math_cycles);
                printf("  | perf_gather_cycles        | %10u |\n", dut->perf_gather_cycles);
                printf("  | perf_slice_cycles         | %10u |\n", dut->perf_slice_cycles);
                printf("  | perf_concat_cycles        | %10u |\n", dut->perf_concat_cycles);
                printf("  | perf_ew_cycles            | %10u |\n", dut->perf_ew_cycles);
                printf("  +---------------------------+------------+\n");
                printf("\n");
            }
        ...
        if (!done) {
            printf("FAIL: Timeout waiting for graph_done (5M cycles exceeded)\n");
            goto cleanup;
        }
    }
